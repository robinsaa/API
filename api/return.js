...
router.post('/', function(req, res, next){
  pool.getConnection(function(err, connection) {
    ...
    // Build query
    var query = 'INSERT INTO ' + table + ' (cup_id, bin_id, cafe_id, scanned_at) VALUES (' + req.body.cup_id + ', ' + req.body.bin_id + ', ' + req.body.cafe_id + ', current_timestamp())';
    console.log(query);
    // Use the connection
    connection.query(query, function (error, result, fields) {
      ...
      // Don't use the connection here, it has been returned to the pool.
      console.log(result);
      res.status(201).send(`Return recorded with ID: ${result.insertId}`);
    });
  });
})
